...
const ContentList: React.FC<IDefautProps, IDefautState> = (props) => {
	...
	//Delete Content Item
	const deleteContentItem = (contentName) => {
		const newContentArray = contentArray.filter((content_item) => content_item.contentName !== contentName);
		const newContent = newContentArray.reduce((acc, item) => {
			return {...acc, [item.contentName]: item.contentItem }
		}, {})
		const updateConfig = {
			...config,
			rules: {
