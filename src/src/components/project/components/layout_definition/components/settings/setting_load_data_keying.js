...
export default class SettingLoadDataSource extends PureComponent {
    handleChangeSource = (event, index, values) => {
            ...
            if(_item){
                return (
                    <MenuItem
                        key={_item.name}
                        insetChildren={true}
                        checked={values && values.indexOf(_item.name) > -1}
                        value={_item.name}
    ...
    render() {
        ...
        const { fieldMap, source } = this.props;
        return (
            <ListItem
                leftCheckbox={
                    <Checkbox
                    ...
                    />}
                disabled={!active}
                primaryText="Keying's source reference"
