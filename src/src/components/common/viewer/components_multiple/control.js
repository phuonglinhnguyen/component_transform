...
export default class ControlCanvas extends Component {
  ...
  drawSections = (ctx) => {
    ...
    if (lineHeightOriginal > 0) {
      ...
      let offsetText = parseInt(lineHeightOriginal * 0.85)
      ctx.fillStyle = 'rgba(251,152,125,1)';
      ctx.font = parseInt(lineHeightOriginal * 0.9) + "px Arial";
      let lengthText = ctx.measureText(lineNo).width
      let lineStartX = -parseInt(lineHeightOriginal) / 2 - lengthText - helfWidth;
    ...
    if (this.loaded)
      try {
        ...
        if (rectangle) {
          ...
          ctx.fillText(this.state.line + 1, rectangle.x - this.imageData.width / 2, rectangle.y - this.imageData.height / 2 - (this.offsetTop + this.offsetTopCurrent) - lineHeightOriginal / 2);
        }
