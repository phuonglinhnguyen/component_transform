...
export default class IOConfiguration {
    ...
    get_info_configurations = (projectId) => {
        return this._conn.get(`apps/${APP_NAME}/projects/${projectId}/${this._api_url}?configurations=${CONFIGURATIONS.join(',')}`)
    }
    get_export_configurations = (projectId, collections, excludes) => {
        ...
        return this._conn.get(`apps/${APP_NAME}/projects/${projectId}/${this._api_url}?data_type=export&&configurations=${collections.join(',')}${excludeRequest}`)
    }
    import_configurations = (projectId, data, collections) => {
        return this._conn.patch(`apps/${APP_NAME}/projects/${projectId}/${this._api_url}?configurations=${collections.join(',')}&&data_type=count`, { type: 'json', data });
    }
    clone_configurations = (projectId, propjectIdrefer, collections, excludes) => {
        let excludeString = transFormExcludesToString(excludes)
        let excludeRequest = excludeString.length ? `&&excludes=${excludeString}` : ''
        return this._conn.patch(`apps/${APP_NAME}/projects/${projectId}/${this._api_url}/clone_from/${propjectIdrefer}?configurations=${collections.join(',')}&&data_type=count${excludeRequest}`, {});
    }
}
