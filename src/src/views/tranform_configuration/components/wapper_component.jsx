			...
			<div className={classes.top}>
				...
				<div className={classes.top}>
					...
					<Button variant="outlined" color="primary" onClick={async () => {
						let project_id = props.history.location.pathname.replace("/projects/", "").replace("/tranform-config","");
			...
			<div style={{ overflowY: 'auto', height: '730px' }}>
				<Table style={{ tableLayout: 'fixed' }}>
					<TableBody className={classes.tableConfig}>
						{configData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((config) => (
							<TableRow
								key={config.name}
