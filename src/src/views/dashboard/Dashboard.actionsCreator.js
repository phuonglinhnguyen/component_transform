...
import { PROJECT_FUNCTIONS} from '../../providers';
...
export const changeProject = (id) => (dispatch, getState) => {
  ...
  if (id) {
    dispatch(crudGetList(PROJECT_FUNCTIONS, { id }))
    dispatch({
      type: DASHBROAD_CHANGE_VALUE_ITEM,
...
export const onClickFunc = (func, project) => (dispatch) => {
    dispatch(push(`/projects/${project.id}${func.path}`))
}

  // ({
  //   type: DASHBROASHBROAD_CHANGE_VALUE_ITEM,
