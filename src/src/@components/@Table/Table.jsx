...
class EnhancedTable extends React.Component {
    ...
    render() {
        const { classes, columns = [],listColumnsSortDefault } = this.props;

        const { data, order, orderBy, rowsPerPage, page } = this.state;
        const emptyRows = rowsPerPage - Math.min(rowsPerPage, data.length - page * rowsPerPage);
        const columnsShow = columns.filter(item => item.visible);

        let dataOrder = orderBy ? orderByFn(data, orderBy, order) : orderByFn(data, listColumnsSortDefault.listColumnsName, listColumnsSortDefault.listtypeSort);
        return (
